...
vm.runInContext(appJs + `\n;globalThis.__exports = { APP, state, selectedScenario, ensureRecord, ensureDriftState, bestHypothesisIndex, bestInterventionIndex, applyRepairToRecord, render, benchmarkForScenario, compositeScore, runDriftResponse, resolveDriftBranch };`, context);

const { APP, state, selectedScenario, ensureRecord, ensureDriftState, bestHypothesisIndex, bestInterventionIndex, applyRepairToRecord, render, benchmarkForScenario, runDriftResponse, resolveDriftBranch } = context.__exports;
...
for(let i=0; i<selectedScenario().phases.length; i++){
...
fs.writeFileSync(path.join(root, 'tests', 'smoke', 'lfc-v08-harness-results.json'), JSON.stringify(results, null, 2));
